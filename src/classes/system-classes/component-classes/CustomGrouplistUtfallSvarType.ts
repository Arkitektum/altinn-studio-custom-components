...
import type { UtfallSvarProps } from "../../data-classes/UtfallSvar.ts";
// Dependencies
...
export default class CustomGrouplistUtfallSvarType extends CustomComponent {
     ...
     * 3. besvart tidligere
     */
    groupArrayItemsByUtfallType(array: unknown): Record<string, UtfallSvar[]> {
        if (!hasValue(array)) return {};

        const grouped = (array as UtfallSvarProps[]).reduce((acc: Record<string, UtfallSvar[]>, obj) => {
        ...
        const getSortPriority = (item: UtfallSvar) => {
    ...
    getResourceBindings(props: ComponentProps) {
        ...
        if (typeof data === "object" && hasValue(data)) {
            for (const utfallType of Object.keys(data as object)) {
