import CustomGroupDispensasjonOversikt from "./CustomGroupDispensasjonOversikt.ts";
...
let mockHasValueImpl = (v: unknown) => v !== undefined && v !== null && v !== "";
jest.mock("@arkitektum/altinn-studio-custom-components-utils", () => ({
    getTextResourceFromResourceBinding: (...args: unknown[]) => mockGetTextResourceFromResourceBinding(...args),
    hasValue: jest.fn((v: unknown) => mockHasValueImpl(v))
}));
...
jest.mock("../../../functions/validations", () => ({
    hasMissingTextResources: (...args: unknown[]) => mockHasMissingTextResources(...args),
    hasValidationMessages: (...args: unknown[]) => mockHasValidationMessages(...args)
}));
jest.mock("../../../functions/helpers", () => ({
    getComponentDataValue: (...args: unknown[]) => mockGetComponentDataValue(...args)
}));

describe("CustomGroupDispensasjonOversikt", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (DispensasjonOversikt as unknown as jest.Mock).mockClear();
    });

    it("should initialize with default resource bindings and values", () => {
        mockGetComponentDataValue.mockReturnValue("mockData");
        (DispensasjonOversikt as unknown as jest.Mock).mockImplementation((data) => ({ data }));
