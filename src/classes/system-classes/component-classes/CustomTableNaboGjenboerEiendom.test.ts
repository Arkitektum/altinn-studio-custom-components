...
import CustomTableNaboGjenboerEiendom from "./CustomTableNaboGjenboerEiendom.ts";
...
jest.mock("../../data-classes/Eiendom.ts", () => {
    return function Eiendom(this: Record<string, unknown>, data: unknown) {
...
jest.mock("../../../functions/validations.ts", () => ({
    ...
    hasValidationMessages: jest.fn((messages: unknown[]) => messages && messages.length > 0)
}));
...
describe("CustomTableNaboGjenboerEiendom", () => {
    ...
    describe("constructor", () => {
        ...
        it("sets resourceValues.data to data when not empty", () => {
            ...
            const instance = new CustomTableNaboGjenboerEiendom(props as unknown as ComponentProps);
    ...
    describe("getValueFromFormData", () => {
        it("returns list of Eiendom objects from formData", () => {
            ...
            const instance = new CustomTableNaboGjenboerEiendom(props as unknown as ComponentProps);
            const result = instance.getValueFromFormData(props as unknown as ComponentProps) as unknown[];

            expect(Array.isArray(result)).toBe(true);
            expect((result as { kommunenavn?: unknown }[])[0]!.kommunenavn).toBe("Oslo");
        });
    ...
    describe("getValidationMessages", () => {
        it("returns missing text resources", () => {
            const instance = new CustomTableNaboGjenboerEiendom({});
            const result = instance.getValidationMessages({} as unknown as Record<string, ResourceBindingGroup>);
    ...
    describe("hasZipCodeOrCity", () => {
        it("returns true if postnr exists", () => {
            (hasValue as unknown as jest.Mock).mockImplementation((val) => val === "1234");
            const instance = new CustomTableNaboGjenboerEiendom({});
            expect(instance.hasZipCodeOrCity({ adresse: { postnr: "1234" } })).toBe(true);
        });

        it("returns true if poststed exists", () => {
            (hasValue as unknown as jest.Mock).mockImplementation((val) => val === "Oslo");
            const instance = new CustomTableNaboGjenboerEiendom({});
    ...
    describe("getResourceBindings", () => {
        it("returns default resource bindings when no props provided", () => {
            ...
            expect(result.adresse!.title).toContain("resource.adresse");
