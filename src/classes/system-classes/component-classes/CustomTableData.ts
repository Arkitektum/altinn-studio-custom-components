...
export default class CustomTableData extends CustomComponent {
     ...
     */
    getTableHeadersFromProps(props: ComponentProps): TableHeader[] {
     ...
     */
    getTableRowsFromProps(props: ComponentProps, data: unknown): CellComponentProps[][] {
        const tableColumns = props?.tableColumns || [];
     ...
     */
    removeEmptyTableRows(tableRows: unknown): CellComponentProps[][] {
        return removeEmptyRows(tableRows) as CellComponentProps[][];
    }
    ...
    hasContent(formData: unknown): boolean {
        ...
        return hasValue((formData as { tableRows?: unknown } | undefined)?.tableRows);
    }
    ...
    getComponentUsage(): string[] {
        return ["custom-feedbacklist-validation-messages", "custom-table"];
    }
}
