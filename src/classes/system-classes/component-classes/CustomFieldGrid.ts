 ...
 */
/** One row of a grid: the label, the component that renders the value, and whether that value came to nothing. */
export interface GridRow {
    term: string | undefined;
    valueComponent: ComponentProps;
    isEmpty: boolean;
}

export default class CustomFieldGrid extends CustomComponent {
    declare resourceValues: { title?: unknown; termHeader?: unknown; valueHeader?: unknown; data?: GridRow[] };
     ...
     */
    getRowsFromProps(props: ComponentProps): GridRow[] {
     ...
     */
    getCellData(column?: TableColumn, formData?: Record<string, unknown>): unknown {
     ...
     */
    hasContent(rows: unknown): boolean {
        return Array.isArray(rows) && rows.some((row: GridRow) => !row.isEmpty);
    }
