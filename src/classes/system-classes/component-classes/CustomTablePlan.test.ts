...
import CustomTablePlan from "./CustomTablePlan.ts";
...
jest.mock("../../data-classes/AndrePlaner.ts", () => {
    return function AndrePlaner(this: Record<string, unknown>, { plan }: { plan?: unknown }) {
        this.plan = plan;
    };
});
...
describe("CustomTablePlan", () => {
    ...
    describe("constructor", () => {
        it("sets isEmpty, validationMessages, hasValidationMessages, resourceBindings, and resourceValues correctly when data is present", () => {
            (getComponentDataValue as unknown as jest.Mock).mockReturnValue("planData");
            (hasValue as unknown as jest.Mock).mockReturnValue(true);
            (hasMissingTextResources as unknown as jest.Mock).mockReturnValue({ some: "validation" });
    ...
    describe("getValidationMessages", () => {
        it("uses empty array if window.textResources is not defined", () => {
            (hasMissingTextResources as unknown as jest.Mock).mockReturnValue("validationResult");
            const instance = new CustomTablePlan({});
            const result = instance.getValidationMessages({ key: "value" } as unknown as Record<string, ResourceBindingGroup>);
