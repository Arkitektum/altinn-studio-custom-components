...
import CustomGroupRammebetingelserTilknytninger from "./CustomGroupRammebetingelserTilknytninger.ts";
...
describe("CustomGroupRammebetingelserTilknytninger", () => {
    beforeEach(() => {
        ...
        (hasValue as unknown as jest.Mock).mockImplementation((val) => val !== undefined && val !== null && val !== "" && val !== false);
    ...
    it("should set isEmpty true and use emptyFieldText when data is empty", () => {
        ...
        (getTextResourceFromResourceBinding as unknown as jest.Mock).mockImplementation((key) => `empty:${key}`);
    ...
    it("should handle undefined props gracefully", () => {
        ...
        // Constructed with nothing on purpose: everything it would read from the props is mocked here.
        const instance = new CustomGroupRammebetingelserTilknytninger(undefined as unknown as ComponentProps);
        expect(instance.isEmpty).toBe(true);
        expect(instance.resourceValues.data).toBe("empty:resource.emptyFieldText.default");
    });
});
