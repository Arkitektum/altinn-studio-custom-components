...
import CustomGrouplistVegtypeTillatelse from "./CustomGrouplistVegtypeTillatelse.ts";
...
jest.mock("@arkitektum/altinn-studio-custom-components-utils", () => ({
    hasValue: jest.fn((data: unknown) => !!data),
    getTextResourceFromResourceBinding: jest.fn((key: string) => `text:${key}`)
}));
jest.mock("../../../functions/validations.ts", () => ({
    hasMissingTextResources: jest.fn((bindings: { vegtype?: { title?: string } }) => {
...
describe("CustomGrouplistVegtypeTillatelse", () => {
    ...
    it("should set isEmpty to false and use data when data is present", () => {
        ...
        const instance = new CustomGrouplistVegtypeTillatelse(props as unknown as ComponentProps);
    ...
    it("should set validationMessages and hasValidationMessages correctly", () => {
        (hasValue as unknown as jest.Mock).mockReturnValue(true);
        (getComponentDataValue as unknown as jest.Mock).mockReturnValue("someData");
        (hasMissingTextResources as unknown as jest.Mock).mockReturnValue(["Missing something"]);
    ...
    it("should omit vegtypeTillatelse resource binding if hideIfEmpty is true", () => {
        ...
        expect(instance.getResourceBindings(props as unknown as ComponentProps).vegtypeTillatelse).toBeUndefined();
    });
    ...
    it("getValidationMessages should delegate to hasMissingTextResources", () => {
        ...
        expect(instance.getValidationMessages({} as unknown as Record<string, ResourceBindingGroup>)).toEqual(["msg"]);
    });

    it("getValueFromFormData should return data from VegtypeTillatelseList.resourceValues", () => {
        (getComponentDataValue as unknown as jest.Mock).mockReturnValue("abc");
        const instance = new CustomGrouplistVegtypeTillatelse({});
        expect(instance.getValueFromFormData({ formData: "abc" } as unknown as ComponentProps)).toBe("mockedData");
    });
