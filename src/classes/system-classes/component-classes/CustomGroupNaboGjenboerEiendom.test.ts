...
import CustomGroupNaboGjenboerEiendom from "./CustomGroupNaboGjenboerEiendom.ts";
...
describe("CustomGroupNaboGjenboerEiendom", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        (NaboGjenboerEiendom as unknown as jest.Mock).mockImplementation((...args: unknown[]) => ({ ...(args[0] as object), _isNabo: true }));
    ...
    it("getValidationMessages delegates to hasMissingTextResources", () => {
        (getTextResources as unknown as jest.Mock).mockReturnValue(["foo"]);
        hasMissingTextResources.mockReturnValue(["missing"]);
        const instance = new CustomGroupNaboGjenboerEiendom({});
