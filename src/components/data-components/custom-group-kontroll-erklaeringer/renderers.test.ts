...
/** The two declaration texts this group shows, each under a binding named for the role it belongs to. */
const textFields: [string, string][] = [
    ["renderErklaeringTekstElement", "kontrollErklaeringTekst"],
    ["renderKONTROLLTekstElement", "kontrollKONTROLLTekst"]
];

/** The declarations of control, which hold nothing but the texts to show. */
const component = {
...
describe("the header", () => {
    ...
    it("sits at h3 unless a level is asked for", () => {
        expect(attributes(renderers.renderHeaderElement("Kontroll", "h2")).size).toBe("h2");
    });
});

describe("the two declaration texts", () => {
    ...
    it("keeps the general declaration apart from the one naming the control role", () => {
...
describe("the empty field text", () => {
    it("renders whatever it was handed as the paragraph's own title", () => {
        expect(attributes(renderers.renderEmptyFieldText({ resourceValues: { data: "Ingen kontrollerklæringer" } }))).toMatchObject({
            tagName: "custom-paragraph",
            wrapped: true,
            resourceValues: { title: "Ingen kontrollerklæringer" }
        });
    });
});
