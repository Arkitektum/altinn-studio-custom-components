...
const component = {
    ...
    resourceValues: { data: [{ adresse: "Storgata 1" }], title: "Eiendom" }
};
...
const column = (rendered, dataKey) => attributes(rendered).columns.find((entry) => entry.dataKey === dataKey);
...
describe("the address column on a landscape page", () => {
    ...
    const widthWhenOriented = (orientation) => {
