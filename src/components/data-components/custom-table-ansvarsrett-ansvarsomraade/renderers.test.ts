...
const area = (kodeverdi: string) => ({ funksjon: { kodeverdi }, beskrivelseAvAnsvarsomraadet: "Beskrivelse" });

/** The table for the given areas, with anything else the case under test puts alongside the data. */
const table = (areas: unknown, extra: Record<string, unknown> = {}) =>
    renderAnsvarsrettAnsvarsomraadeTable({ resourceBindings, resourceValues: { data: areas, ...extra } });
...
const columnKeys = (areas: unknown, extra?: Record<string, unknown>) => attributes(table(areas, extra)).columns.map((entry) => entry.dataKey);
