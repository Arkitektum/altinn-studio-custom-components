 ...
 */
export function renderAnsvarsrettAnsvarsomraadeTable(component?: InstantiatedComponent | null) {
...
function setErklaeringTitle(component: InstantiatedComponent | null | undefined, funksjonListe: string[]) {
...
function setFunksjonListe(component?: InstantiatedComponent | null) {
    const funksjonListe: string[] = [];
    if (component?.resourceValues?.data && Array.isArray(component?.resourceValues.data)) {
        component?.resourceValues.data.forEach((element: { funksjon?: { kodeverdi?: unknown } }) => {
