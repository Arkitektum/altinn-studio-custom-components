...
const textsOf = (element, className) => [...element.querySelectorAll(`.${className}`)].map((node) => node.textContent);
...
describe("the rows", () => {
    ...
    it("puts the value in an element of its own, of the kind the row asked for", () => {
        ...
        expect(values.map((node) => node.firstChild.tagName.toLowerCase())).toEqual(["custom-field-data", "custom-field-adresse"]);
    });
    ...
    it("hands each row's own properties to the element holding its value", () => {
        ...
        expect(JSON.parse(value.firstChild.getAttribute("resourcevalues"))).toEqual({ data: "100" });
    });
