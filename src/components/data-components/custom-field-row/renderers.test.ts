import { renderFieldRowElement } from "./renderers.ts";
...
describe("the cells", () => {
    ...
    it("puts each value in the kind of element its entry asked for", () => {
        ...
        expect(cells.map((cell) => (cell.firstChild as HTMLElement).tagName.toLowerCase())).toEqual(["custom-field-data", "custom-field-adresse"]);
    });
    ...
    it("styles the cell itself rather than the element inside it", () => {
        ...
        const cell = renderFieldRowElement(component).querySelector(".custom-field-row-cell") as HTMLElement | null;
    ...
    it("hands each entry's own properties to the element inside its cell", () => {
        ...
        expect(JSON.parse((cell!.firstChild as HTMLElement).getAttribute("resourcevalues")!)).toEqual({ data: "100" });
    });
});
