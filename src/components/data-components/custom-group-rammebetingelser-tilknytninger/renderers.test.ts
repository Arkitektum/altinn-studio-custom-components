...
import * as overvannRenderers from "../custom-group-overvann/renderers.ts";
...
import * as vannforsyningRenderers from "../custom-group-vannforsyning/renderers.ts";
...
const handOver = (render: (component?: InstantiatedComponent | null) => HTMLElement) => {
...
describe("the four connections", () => {
    ...
    it.each(connections)("%s renders %s with its own slice of the data", (name, tagName, key) => {
        expect(attributes(byName[name]!(component))).toMatchObject({
            ...
            hideIfEmpty: "true",
            wrapped: false,
            resourceValues: { data: (component.resourceValues.data as Record<string, unknown>)[key] },
            resourceBindings: { title: (component.resourceBindings as Record<string, { title?: string }>)[key]!.title }
        });
    });
});
...
describe("what the groups below make of it", () => {
    ...
    it("gives the access group a title it can find", () => {
        ...
        const bindings = ((rendered.firstChild as HTMLElement).firstChild as HTMLElement).getAttribute("resourcebindings");

        expect(JSON.parse(bindings!).title).toBe("Ny eller endret adkomst");
    });
    ...
    it("gives the sewage group a title it can find", () => {
        ...
        expect(JSON.parse(bindings!).title).toBe("Tilknytningstype for avløp");
    });
    ...
    it("gives the surface water group a title it can find", () => {
        ...
        expect(JSON.parse(bindings!).title).toBe("Ledes til terreng");
    });
    ...
    it("gives the water supply group a title it can find", () => {
        ...
        expect(JSON.parse(bindings!).title).toBe("Beskrivelse av vannforsyning");
    });
});
