...
const booleanFields: [string, string, ...unknown[]][] = [
    ["renderKrysserVannforsyningAnnensGrunnElement", "krysserVannforsyningAnnensGrunn", true],
    ["renderHarTinglystErklaeringElement", "harTinglystErklaering", false]
];

/** The water supply, with everything its renderers read. */
const component = {
    resourceBindings: {
    ...
    resourceValues: {
        data: {
            tilknytningstype: { kodebeskrivelse: "Privat", kodeverdi: "PRI" },
            beskrivelse: "Egen brønn",
...
describe("the header", () => {
    it("takes the title it is given and sits at h3 unless a level is asked for", () => {
        expect(attributes(renderers.renderHeaderElement("Vannforsyning"))).toMatchObject({
            tagName: "custom-header-text",
            size: "h3",
            resourceValues: { title: "Vannforsyning" }
        });
        expect(attributes(renderers.renderHeaderElement("Vannforsyning", "h2")).size).toBe("h2");
    });
});
...
describe("the described fields", () => {
    ...
    it("shows the description as it was written", () => {
        expect(attributes(renderers.renderBeskrivelseElement(component))).toMatchObject({
            tagName: "custom-field-data",
            resourceValues: { data: "Egen brønn" }
        });
    });
});
...
describe("every renderer", () => {
    ...
    it("marks everything it renders as a child component", () => {
        ...
        expect(attributes(renderers.renderHeaderElement("Vannforsyning")).isChildComponent).toBe("true");
    });
});
