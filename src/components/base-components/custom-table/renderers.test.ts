import { renderHeaderElement, renderTableElement } from "./renderers.ts";
...
const caption = (table: HTMLTableElement) => table.querySelector("caption");

describe("the caption", () => {
    it("is drawn from the title, and marks the table as having one", () => {
        const table = renderTableElement(component);

        expect((caption(table)!.firstChild as HTMLElement).tagName.toLowerCase()).toBe("custom-header");
        expect(table.classList.contains("has-caption")).toBe(true);
    });

    it("carries the title and the heading level into the header element", () => {
        const header = caption(renderTableElement(component))!.firstChild as HTMLElement;
...
describe("the rows", () => {
    it("draws one cell per entry, each holding the element that entry asked for", () => {
        ...
        expect(cells.map((td) => (td.firstChild as HTMLElement).tagName.toLowerCase())).toEqual(["custom-field-adresse", "custom-field-data"]);
    });

    it("falls back to a plain data field for a cell that names no element", () => {
        const cell = renderTableElement(component).querySelectorAll("tbody td")[1]!;
        expect((cell.firstChild as HTMLElement).tagName.toLowerCase()).toBe("custom-field-data");
    });
    ...
    it("hands each cell's own properties to the element inside it", () => {
        ...
        expect(JSON.parse((cell!.firstChild as HTMLElement).getAttribute("resourcevalues")!)).toEqual({ data: "Storgata 1" });
    });
});
