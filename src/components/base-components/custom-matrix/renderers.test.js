import { renderHeaderElement, renderMatrixElement } from "./renderers.js";
...
describe("the caption", () => {
    it("is drawn from the title, and marks the matrix as having one", () => {
        ...
        expect(matrix.querySelector("caption").firstChild.tagName.toLowerCase()).toBe("custom-header");
...
describe("the rows", () => {
    it("makes the first cell of each row a heading for that row", () => {
        ...
        for (const row of rows) {
            expect({ tagName: row.firstChild.tagName, scope: row.firstChild.getAttribute("scope") }).toEqual({ tagName: "TH", scope: "row" });
        }
    });
    ...
    it("sets the row heading in bold, and leaves the cells beside it alone", () => {
        const row = renderMatrixElement(component()).querySelector("tbody tr");
        const styleOf = (cell) => JSON.parse(cell.firstChild.getAttribute("styleoverride") ?? "null");
    ...
    it("keeps a row heading's own styling alongside the bold it adds", () => {
        ...
        data.matrixRows[0][0].styleOverride = { textAlign: "right" };
        const row = renderMatrixElement({ resourceValues: { data } }).querySelector("tbody tr");

        expect(JSON.parse(row.firstChild.firstChild.getAttribute("styleoverride"))).toEqual({
            textAlign: "right",
...
describe("a matrix with nothing to show", () => {
    ...
    const emptyMatrix = (data) => renderMatrixElement({ resourceValues: { data, emptyFieldText: "Ingen arealer" }, isEmpty: true });
