 ...
 */
function renderMatrixColumnHeaderElement(matrixHeader: TableHeader) {
    const th = document.createElement("th");
    th.setAttribute("scope", "col");
    th.textContent = matrixHeader.text as string;
    return th;
}
 ...
 */
function renderMatrixCellElement(matrixCell: CellComponentProps) {
 ...
 */
function renderMatrixRowElement(matrixRow: CellComponentProps[]) {
    const tr = document.createElement("tr");
    if (matrixRow?.length) {
        matrixRow.forEach((matrixCell, index: number) => {
            if (index === 0) {
