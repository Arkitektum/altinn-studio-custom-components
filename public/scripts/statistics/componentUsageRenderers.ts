 ...
 */
function getAppUsageCountForComponent(component: LayoutComponent | undefined) {
    if (!component?.usages) {
        return 0;
    }
    const uniqueAppIds = new Set(component.usages.map((usage: ApiValue) => `${usage?.appOwner}-${usage?.appName}`));
 ...
 */
function groupUsagesByAppAndLayout(usages: ApiValue, predicate: (usage: ApiValue) => unknown) {
    if (!Array.isArray(usages)) {
        return [];
    }
    const appUsageMap: Record<string, ApiValue> = {};
    usages.forEach((usage: ApiValue) => {
        if (!predicate(usage)) {
            return;
        }
    ...
    return Object.values(appUsageMap).map((app: ApiValue) => ({
 ...
 */
function getDirectComponentUsages(component: LayoutComponent | undefined) {
    if (!component?.usages) {
        return [];
    }
    return groupUsagesByAppAndLayout(component.usages, (usage: ApiValue) => usage?.id);
}
 ...
 */
function getIndirectComponentUsages(component: LayoutComponent | undefined) {
    if (!component?.usages) {
        return [];
    }
 ...
 */
function renderAppUsingComponentListItem(appUsage: ApiValue) {
 ...
 */
function getTotalUsageCountForLayouts(layouts: ApiValue) {
    if (!Array.isArray(layouts)) {
        return 0;
    }
    return layouts.reduce((total: number, layout: Layout | undefined) => total + (layout?.usages?.length || 0), 0);
}
 ...
 */
function renderLayoutUsageDetailsListItem(layoutUsage: ApiValue, renderUsageItem: (usage: ApiValue) => HTMLElement) {
    ...
    layoutUsage?.usages?.forEach((usage: ApiValue) => {
 ...
 */
function renderAppUsageDetailsListItem(appUsage: ApiValue) {
    ...
    appUsage?.layouts?.forEach((layoutUsage: ApiValue) => {
 ...
 */
function renderComponentUsingComponentListItem(componentUsage: ApiValue) {
 ...
 * @returns {HTMLElement} - The details element representing the app usage and its layouts.
 */
function renderComponentUsageDetailsListItem(componentUsage: ApiValue) {
    ...
    componentUsage?.layouts?.forEach((layoutUsage: ApiValue) => {
 ...
 */
export function renderComponentUsageListItem(component: LayoutComponent | undefined) {
    ...
    if (directComponentUsages?.length > 0) {
        ...
        directComponentUsages.forEach((appUsage: ApiValue) => {
    ...
    if (indirectComponentUsages?.length > 0) {
        ...
        indirectComponentUsages.forEach((componentUsage: ApiValue) => {
 ...
 */
export function renderComponentUsageList(componentUsage: ApiValue) {
    ...
    componentUsage.forEach((component: LayoutComponent | undefined) => {
...
function renderLabelledSelectFilter(labelText: string, selectId: string, options: { value: string; text: string }[], onChange: (value: string) => void
    ...
    options.forEach((option: ApiValue) => {
 ...
 */
export function renderSelectApplicationFilterForComponentUsageList(
    ...
    (applications || []).forEach((app: ApiValue) => {
 ...
 */
export function renderTextInputFilterForComponentUsageList(containerElement: HTMLElement, components: LayoutComponent[] | undefined) {
