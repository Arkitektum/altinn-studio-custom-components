...
describe("fetchDisplayLayouts", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ foo: 1 }) });
...
describe("fetchPackageVersions", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ bar: 2 }) });
...
describe("fetchAppResources", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ baz: 3 }) });
...
describe("fetchExampleData", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ ex: 5 }) });
...
describe("fetchAltinnStudioForms", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ forms: 6 }) });
...
describe("fetchApplicationMetadata", () => {
    it("returns JSON on ok", async () => {
        (fetch as unknown as jest.Mock).mockResolvedValue({ ok: true, json: () => Promise.resolve({ meta: 7 }) });
