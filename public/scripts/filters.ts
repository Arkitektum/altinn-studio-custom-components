import type { ApiValue, LayoutComponent } from "./types.ts";
/**
 ...
 */
export function filterResources(resources: ApiValue, filterValue: string) {
    switch (filterValue) {
        case "unused":
            return resources.filter((res: ApiValue) => res?.usage?.length === 0 && res?.missingFromDefaultTextResources !== true);
        case "used-once":
            return resources.filter((res: ApiValue) => res?.usage?.length === 1 && res?.missingFromDefaultTextResources !== true);
        case "with-duplicates":
            return resources.filter(
                (res: ApiValue) => getResourcesWithSameValue(resources, res).length > 0 && res?.missingFromDefaultTextResources !== true
            );
        case "deprecated":
            return resources.filter((res: ApiValue) => res?.resource?.replacedWithId?.length > 0);
        case "missing":
            return resources.filter((res: ApiValue) => res?.presence === "missing");
        case "missing-with-local-value":
            return resources.filter((res: ApiValue) => res?.presence === "localValue");
        case "missing-nb-translations":
            return resources.filter((res: ApiValue) => !res?.resource?.values?.nb);
        case "missing-nn-translations":
            return resources.filter((res: ApiValue) => !res?.resource?.values?.nn);
        case "missing-en-translations":
            return resources.filter((res: ApiValue) => !res?.resource?.values?.en);
        case "all":
            return resources.filter((res: ApiValue) => res?.missingFromDefaultTextResources !== true);
        default:
            return resources;
    }
}
 ...
 */
export function filterTextResourcesByTextInput(resources: ApiValue, textFilter: string, matchBy: string) {
    ...
    return resources.filter((res: ApiValue) => {
        if (matchBy === "id") {
            ...
            return Object.values(res?.resource?.values || {}).some((value: ApiValue) =>
                value?.toString().toLowerCase().includes(lowerCaseTextFilter)
            );
        }
        return false;
    });
 ...
 */
export function getResourcesWithSameValue(resources: ApiValue, resource: ApiValue) {
    return Array.isArray(resources) && resources.length > 0
        ? resources.filter((res: ApiValue) => {
              return (
 ...
 */
export function getComponentType(tagName: string) {
 ...
 */
export function filterComponentsByUsage(components: LayoutComponent[] | undefined, filterValue: string) {
    switch (filterValue) {
        case "unused":
            return components!.filter((component: LayoutComponent | undefined) => (component?.usages?.length || 0) === 0);
        case "used-once":
            return components!.filter((component: LayoutComponent | undefined) => (component?.usages?.length || 0) === 1);
        default:
            return components;
    }
}
 ...
 * @returns {Array<Object>} The filtered array of components.
 */
export function filterComponentsByType(components: LayoutComponent[] | undefined, type: string) {
    if (!type) {
        return components;
    }
    return components!.filter((component: LayoutComponent | undefined) => getComponentType(component?.tagName as string) === type);
}
 ...
 * @returns {Array<Object>} The filtered array of components.
 */
export function filterComponentsByTextInput(components: LayoutComponent[] | undefined, textFilter: string, matchBy: string) {
    if (!textFilter?.length) {
        return components;
    }
    const lowerCaseTextFilter = textFilter.toLowerCase();
    return components!.filter((component: LayoutComponent | undefined) => {
        if (matchBy === "id") {
            return component?.usages?.some((usage: ApiValue) => usage?.id?.toString().toLowerCase().includes(lowerCaseTextFilter));
        }
