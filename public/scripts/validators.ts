...
// Local functions
import { getDataModels, getLayoutCode, getTextResources } from "./localStorage.ts";

// Global functions
import { getDefaultValueForResource } from "./getters.ts";
 ...
 * @returns {void}
 */
function addResourceBindingsFromCustomComponent(componentProps: ApiValue, allResourceBindings: ApiValue) {
    ...
    // Only some component classes work their bindings out for themselves; the rest carry them as given.
    const resourceBindings = (component as { getResourceBindings?: (props: ApiValue) => ApiValue })?.getResourceBindings?.(componentProps);
 ...
 */
function addTableColumnsResourceBindingsFromCustomComponent(componentProps: ApiValue, allResourceBindings: ApiValue) {
    if (componentProps?.tableColumns) {
        for (const tableColumn of Object.values(componentProps.tableColumns) as ApiValue[]) {
 ...
 */
function addResourceBindingsFromAltinnComponent(componentProps: ApiValue, allResourceBindings: ApiValue) {
 ...
 */
function getUnusedResourceBindings(allResourceBindings: ApiValue, textResources: ApiValue) {
    const unusedResourceBindings = [];
    const textResourceIds = textResources?.resources?.map((res: ApiValue) => res.id) || [];
 ...
 */
function getDuplicateTextResources(textResources: ApiValue) {
 ...
 */
export function getMissingResourceBindings(allResourceBindings: ApiValue, textResources: ApiValue, defaultTextResources: ApiValue) {
    const missingResourceBindings = [];
    const literalValues = [];
    const textResourceIds = textResources?.resources?.map((res: ApiValue) => res.id) || [];
    const defaultTextResourceIds = defaultTextResources?.map((res: ApiValue) => res.id) || [];
 ...
 */
function getTextResourcesWithEmptyValue(textResources: ApiValue) {
 ...
 */
export function getResourceBindingsFromComponent(resourceBindingsSet: ApiValue, componentProps: ApiValue, componentType = "all") {
 ...
 */
export function getResourceBindingsFromComponents(resourceBindingsSet: ApiValue, componentsArray: ApiValue, componentType = "all") {
 ...
 */
export function getResourceBindingsFromLayout(resourceBindingsSet: ApiValue, layout: Layout | undefined, componentType = "all") {
 ...
 */
export function getResourceBindingsWithUsageFromApplications(applications: ApiValue, componentType = "all") {
...
export function validateResources() {
    ...
    const defaultTextResources = getDefaultTextResources() as ApiValue;

    const normalizedComponentCode = componentCode ?? [];
    const components = Array.isArray(normalizedComponentCode) ? normalizedComponentCode : [componentCode];
 ...
 */
export function renderValidationMessages(validationResults: ApiValue) {
    const validationMessages = new ValidationMessages({
        error: validationResults.missingResourceBindings
            ...
            .concat(validationResults.duplicateTextResources.map((resId: string) => `Duplicate resource: ${resId}`)),
        warning: validationResults.unusedResourceBindings
            .map((resId: string) => `Unused resource: ${resId}`)
            .concat(validationResults.literalValues.map((resId: string) => `Literal value: ${resId}`)),
        info: validationResults.emptyTextResources.map((resId: string) => `Empty resource: ${resId}`)
    });
 ...
 */
export function resourceIsUsedInComponent(component: LayoutComponent | undefined, resource: ApiValue) {
 ...
 */
export function getResourceUsageForLayout(layout: Layout | undefined, resource: ApiValue) {
 ...
 */
export function getResourceUsage(layouts: ApiValue, resource: ApiValue) {
    const appUsageMap: Record<string, ApiValue> = {};
    layouts.forEach((layout: Layout | undefined) => {
 ...
 */
export function getUsageForResources(layouts: ApiValue, resources: ApiValue) {
    return resources.map((resource: ApiValue) => ({
        resource,
 ...
 */
function getAppResourceForLayout(layout: Layout | undefined, resource: ApiValue, appResourceValues: ApiValue) {
    const appResourceForLayout =
        appResourceValues?.find((res: ApiValue) => res.appName === layout!.appName && res.appOwner === layout!.appOwner) || null;
    return appResourceForLayout?.resourceValues?.find((res: ApiValue) => res.id === resource.id);
}
 ...
 */
function getMissingResourceUsage(layouts: ApiValue, resource: ApiValue, appResourceValues: ApiValue) {
    const missingResourceUsage: ApiValue[] = [];
    const missingResourceUsageWithLocalValue: ApiValue[] = [];
    ...
    layouts.forEach((layout: Layout | undefined) => {
        ...
        if (componentsUsingResource.length > 0) {
            ...
            if (localAppResource) {
                ...
                const existingEntry = missingResourceUsageWithLocalValueMap.get(key);
                if (existingEntry) {
 ...
 */
export function getUsageForMissingResources(layouts: ApiValue, missingResourceBindings: ApiValue, appResourceValues: ApiValue) {
    const missingResourcesUsage: ApiValue[] = [];
    const missingResourcesWithLocalValueUsage: ApiValue[] = [];
    missingResourceBindings.forEach((resourceId: string) => {
